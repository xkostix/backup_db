...
Инкрементное резервное копирование в MySQL выполняется только на уровне физических файлов данных с помощью специализированных инструментов, таких как Xtrabackup (бесплатно) или MySQL Enterprise Backup (коммерческая версия).

Нужно включить бинарное логирование в конфигурации MySQL. Создается первая, полная копия всей базы данных. Последующие бэкапы сохраняют только те данные, которые изменились с момента последней (полной или инкрементной) копии. Восстановление: Для восстановления необходимо иметь последнюю полную копию и все последующие инкрементные копии в цепочке.
